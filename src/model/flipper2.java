...
public class flipper2 implements KeyListener{
	...
	public flipper2(String id, int j, int k){
        ...
        cornerCircle = new circle(new Circle((xpos1*25)+6,(ypos1*25)+6,5));
        movingCircle = new circle(new Circle((xpos1*25)+7,(ypos1*25)+44,5));
...
//		setRectangle();
	...
	public void setRectangle1(){
		if(flipped == false){
	        rectangle = new Square(xpos1 * 25, ypos1 * 25, 50, 12, ID, x,y);    //creates a new square object for the underlying gizmo
	        movingCircle = new circle(new Circle((xpos1*25)+45,(ypos1*25)+7,5));
	        flipped = true;
		}
	}
	...
	public void setRectangle2(){
		if(flipped == true){
	        rectangle = new Square(xpos1 * 25, ypos1 * 25, 12, 50, ID, x,y);    //creates a new square object for the underlying gizmo
	        movingCircle = new circle(new Circle((xpos1*25)+7,(ypos1*25)+44,5));
			flipped = false;
